...
class ModelPaymentCoinpayments extends Model
{
    ...
    public function createInvoice($order_info)
    {
        ...
        if ($this->config->get('coinpayments_webhooks')) {
            $invoice = $this->coinpayments->createMerchantInvoice($client_id, $client_secret, $coin_currency['id'], $invoice_id, $amount, $display_value);
        } else {
            $invoice = $this->coinpayments->createSimpleInvoice($client_id, $coin_currency['id'], $invoice_id, $amount, $display_value);
        }
    ...
    public function checkDataSignature($signature, $content)
    {
        ...
        $request_url = $this->coinpayments->getNotificationUrl();
        $client_secret = $this->config->get('coinpayments_client_secret');
