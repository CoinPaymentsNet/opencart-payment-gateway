...
class ModelPaymentCoinpayments extends Model
{
    ...
    public function validateWebhook($client_id, $client_secret)
    {
        ...
        if (!empty($webhooks_list)) {
            ...
            if (!in_array($this->coinpayments->getNotificationUrl($client_id, Coinpayments::PAID_EVENT), $webhooks_urls_list) || !in_array($this->coinpayments->getNotificationUrl($client_id, Coinpayments::CANCELLED_EVENT), $webhooks_urls_list) || !in_array($this->coinpayments->getNotificationUrl($client_id, Coinpayments::PENDING_EVENT), $webhooks_urls_list)) {
                if (!empty($this->coinpayments->createWebHook($client_id, $client_secret, Coinpayments::PAID_EVENT)) && !empty($this->coinpayments->createWebHook($client_id, $client_secret, Coinpayments::CANCELLED_EVENT)) && !empty($this->coinpayments->createWebHook($client_id, $client_secret, Coinpayments::PENDING_EVENT))) {
                    $valid = true;
                }
            } else {
                $valid = true;
